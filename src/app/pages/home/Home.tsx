...
const Home = () => {
  const girlGroups = ["TWICE", "NMIXX", "NewJeans", "aespa", "LE SSERAFIM"]
  const [ newGirlGroup, setGirlNewGroup ] = useState("Loading...")

  const shuffleGirlGroups = useCallback(() => {
    const index = Math.floor(Math.random() * girlGroups.length)
    setGirlNewGroup(girlGroups[index])
  }, [])
      ...
      <section id="home" className="container mx-auto relative top-[400px] h-full lg:top-[350px] 2xl:top-[300px]">
        <div className="ads w-[400px] flex flex-col items-end gap-y-[50px] ml-[40px] sm:w-[500px] sm:ml-[50px] lg:mx-0 lg:ml-[75px] xl:ml-[100px] 2xl:ml-[150px] premium:w-[600px] premium:ml-[200px] ">
          <div className="text relative flex flex-col justify-end gap-y-[8px]">
            <h1 className="title text-[2.5rem] font-[700] leading-[3rem] tracking-tight sm:text-[3.125rem] sm:leading-[3.75rem] md:text-[4rem] md:leading-[4.8rem] 2xl:text-[4.875rem] 2xl:leading-[5.85rem] premium:text-[6rem] premium:leading-[7.2rem]">{newGirlGroup}</h1>
